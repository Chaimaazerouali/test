 ...
 */
int showAliases(data_of_program *data, char *aliasName)
{
    int i, aliasNameLength, aliasFound = 0;

    if (data->alias_list)
    {
        aliasNameLength = str_length(aliasName);
        for (i = 0; data->alias_list[i]; i++)
        {
            if (!aliasName || (str_compare(data->alias_list[i], aliasName, aliasNameLength)
 ...
 */
char *getAliasValue(data_of_program *data, char *aliasName)
{
    ...
    for (i = 0; data->alias_list[i]; i++)
    {
        if (str_compare(aliasName, data->alias_list[i], aliasNameLength) &&
            data->alias_list[i][aliasNameLength] == '=')
        {
 ...
 * Return: 0 if success, 1 otherwise.
 */
int setAlias(char *aliasString, data_of_program *data)
{
